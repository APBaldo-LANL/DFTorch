...
def get_h_spin(TYPE, net_spin, w, n_shells_per_atom, shell_types):
    ...
    if w.dim() == 1:
        w_tmp = w[TYPE[mask_tmp1]].unsqueeze(-1).unsqueeze(-1) * torch.ones((1,1), device=net_spin.device)
    else:
        w_tmp = w[TYPE[mask_tmp1]][:,0:1,0:1]
    mask_tmp = mask_tmp1.repeat_interleave(n_shells_per_atom) # map atom inds onto orbitals
    ...
    if w.dim() == 1:
        w_tmp = w[TYPE[mask_tmp1]].unsqueeze(-1).unsqueeze(-1) * torch.ones((2,2), device=net_spin.device)
    else:
        w_tmp = w[TYPE[mask_tmp1]][:,0:2,0:2]
    mask_tmp = mask_tmp1.repeat_interleave(n_shells_per_atom) # Generate atom index for each orbital
    ...
    if w.dim() == 1:
        w_tmp = w[TYPE[mask_tmp1]].unsqueeze(-1).unsqueeze(-1) * torch.ones((3,3), device=net_spin.device)
    else:
...
def get_spin_energy(TYPE, net_spin, w, n_shells_per_atom):
    ...
    mask_tmp = mask_tmp1.repeat_interleave(n_shells_per_atom) # Generate atom index for each orbital
    q_tmp = net_spin[mask_tmp].view(-1,2)
    e_spin = e_spin + (q_tmp.unsqueeze(1).transpose(-1,-2) * w_tmp * q_tmp.unsqueeze(1)).sum()
...
def get_spin_energy_shadow(TYPE, net_spin, n_net_spin, w, n_shells_per_atom):
    ...
    e_spin = e_spin + (q_tmp1.unsqueeze(1).transpose(-1,-2) * w_tmp * q_tmp2.unsqueeze(1)).sum()
