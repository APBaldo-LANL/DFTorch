...
def scf_x_os(
    ...
    with torch.no_grad():
        ...
        Dorth, Q, e, f, mu0 = dm_fermi_x_os(Z.T @ H0 @ Z, Te, Nocc, mu_0=None, eps=1e-9, MaxIt=50, broken_symmetry=broken_symmetry)
        ...
        while ((ResNorm > dftorch_params['SCF_TOL']) or (dEc > dftorch_params['SCF_TOL']*100)) and it < dftorch_params['SCF_MAX_ITER']:
            ...
            q_tot_atom.scatter_add_(0, shell_to_atom, q_spin_sr.sum(dim=0)) # atom-resolved
            net_spin_sr = q_spin_sr[0] - q_spin_sr[1]
            net_spin_atom = torch.zeros_like(RX)
            net_spin_atom.scatter_add_(0, shell_to_atom, net_spin_sr)
            ...
            dEc = torch.abs(Ecoul_old-Ecoul)
            print(f"current atomic charges: {q_tot_atom}")
            print(f"current atomic spins: {net_spin_atom}")
            print(f"chemical potentials: {mu0}")
    ...
    print(f"current atomic charges sr: {q_spin_sr}")
    print(f"current atomic charges: {q_tot_atom}")
    print(f"current atomic spins: {net_spin_atom}")
